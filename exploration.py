...
def update_network_graph(scatterplot_input: dict, networkgraph_input: dict, elements: list):
    ...
    if networkgraph_input is not None:
        ...
        if networkgraph_input["id"] not in node_extensions:
            ...
            remove_selected_node: bool = networkgraph_input["id"].startswith("W")

            edge_targets_and_sources = []

            for element_dict in elements:  # Collapse wiki extensions on second click
                # Process nodes.
                if "id" in element_dict["data"] and "source" not in element_dict["data"]:
                    # Delete nodes generated by selected node.
                    if element_dict["data"].get("id") in node_extensions[networkgraph_input["id"]]:
                        elements.remove(element_dict)

                # Process edges.
                if "source" in element_dict["data"]:
                    # Delete edges originating from selected node.
                    if element_dict["data"].get("target") in node_extensions[networkgraph_input["id"]]:
                        elements.remove(element_dict)

                # Remove node extension entries.
            del node_extensions[networkgraph_input["id"]]

            # Collect target and source ids from edges
            for element_dict in elements:
                # Process edges.
                if "source" in element_dict["data"]:
                    edge_targets_and_sources.append(element_dict["data"]["target"])
                    edge_targets_and_sources.append(element_dict["data"]["source"])

            # Removed nodes with no edges.
            for element_dict in elements:
                # Process nodes.
                if "id" in element_dict["data"] and "source" not in element_dict["data"]:
                    if element_dict["data"]["id"] not in edge_targets_and_sources:
                        print(element_dict)
                        elements.remove(element_dict)
