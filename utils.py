...
def collapse_node_in_network_graph(
        elements: list, original_node_id, node_extensions: dict, delete_original_node: bool = False
) -> Tuple[list, dict]:
    """
    Removes subtree of elements from graph. Assumes tree structure. Note: Modifies elements and node_extionsions
    directly.
    :param elements: List of nodes and edges.
    :param original_node_id: ID of node whose subtree to delete.
    :param node_extensions: Dictionary holding relevant node lineage.
    :param delete_original_node: Whether or not to delete the original node as well or just its subtree.
    :return: List of elements with removed subtree; updated dictionary of node extensions.
    """

    for element in elements:  # Collapse wiki extensions on second click
        # Delete nodes generated by selected node.
        if "source" not in element["data"]:
            # Remove subnode.
            if element["data"].get("id") in node_extensions[original_node_id]:
                elements.remove(element)

            # Remove original node, if requested.
            if delete_original_node and element["data"].get("id") == original_node_id:
                elements.remove(element)

        # Delete edges originating from selected node.
        else:
            if element["data"].get("target") in node_extensions[original_node_id]:
                elements.remove(element)

    # Call for subtrees.
    for subtree_startnode in node_extensions[original_node_id]:
        if subtree_startnode in node_extensions:
            collapse_node_in_network_graph(elements, subtree_startnode, node_extensions, True)

    # Remove node extension entries.
    del node_extensions[original_node_id]

    return elements, node_extensions
